...
def get_description_urls(item):
    ...
    if item['description']:
        return re.findall(r'(https?://\S+)', item['description'])
    else:
        return []
...
def dump_vector(vector=None):
    ...
    if vector is None:
        ...
        split = pd.read_csv(f'datasets/{dataset}/split.csv', header=0)
        ...
        train_uid = set(split[split["split"].isin(["train","val"])]["id"])
...
if __name__ == '__main__':
    with open('{}/node.json'.format(path) if dataset != 'Twibot-22' else '{}/user.json'.format(path)) as f:
        ...
        for item in tqdm(data, ncols=0):
            ...
            name = item['name']
            if name is None and screen_name is None:
                continue
            ...
            if item['url']:
                user_vector["profile_url"] = 1 if len(re.findall(r'(https?://\S+)', str(item['url']))) > 0 else 0
            else:
                user_vector["profile_url"] = 0
